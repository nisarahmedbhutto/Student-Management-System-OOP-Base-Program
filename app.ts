...
interface StudentInfo {
  students: Map<number, StudentData>;
}
...
async function addNewStudent(
  ...
  console.log(
    chalk.blueBright.bold(
      `Student Name Is "${studentData.name}" ::: Selected Course "${studentData.courses}"`
    )
  );
...
async function checkBalance(students: Map<number, StudentData>) {
  let studentIdResponse = await inquirer.prompt({
    name: "studentId",
    type: "input",
    message: chalk.bgBlueBright.bold(
      `Enter Student ID to check balance...! (${StudentData.counter})`
    ),
  });
  ...
  if (student) {
    console.log(
      chalk.blueBright.bold(
        `Balance for Student ${student.studentBalance}:  Student ID : ${student.studentId}`
      )
    );
  } else {
...
async function payFees(students: Map<number, StudentData>) {
  let studentIdResponse = await inquirer.prompt({
    name: "studentId",
    type: "input",
    message: chalk.bgBlueBright.bold(
      `Enter Student ID to pay fees...! (${StudentData.counter}`
    ),
  });
  let student = students.get(parseInt(studentIdResponse.studentId));
  if (student) {
    ...
    if (studentData.payFees > student.studentBalance) {
      ...
      console.log(
        chalk.blueBright.bold(
          `Payment of ${studentData.payFees} made for Student ID ${student.studentId}. Updated balance is ${student.studentBalance}`
        )
      );
    }
  } else {
...
async function showAllData(students: Map<number, StudentData>) {
  students.forEach((student) => {
    console.log(
      chalk.bgBlueBright.bold.italic(
        "--------------Student Managment System Nisar Ahmed Bhutto------------------------"
      )
    );
...
async function manageStudents() {
  let studentInfo: StudentInfo = { students: new Map<number, StudentData>() };
  console.log(
    chalk.bgBlueBright.bold.italic(
      "--------------Student Managment System Nisar Ahmed Bhutto------------------------"
    )
